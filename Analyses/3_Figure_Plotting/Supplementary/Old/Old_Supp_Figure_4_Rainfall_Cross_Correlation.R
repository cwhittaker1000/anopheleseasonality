...
for (i in 1:length(fitted_mosquito_data[, 1])) {
  if (i == 1) {
    plot(timepoints, monthly_sum_rainfall_storage[i, ], type = "l", ylim = c(0, 400), xaxt = "n",
         col = adjustcolor("black", alpha.f = 0.1), xlab = "Month", ylab = "Rainfall (mm)", las = 1)
...
mean_monthly_sum <- apply(monthly_sum_rainfall_storage, 2, mean)
lines(timepoints, mean_monthly_sum, type = "l", col = "red", lwd = 5)
...
for (i in 1:4) {
  ...
  plot(timepoints, apply(temp, 2, mean) * 100, type = "l", yaxt = "n", lwd = 2, ylim = c(0, 10), col = palette()[i], las = 1, xaxt = "n")
}
