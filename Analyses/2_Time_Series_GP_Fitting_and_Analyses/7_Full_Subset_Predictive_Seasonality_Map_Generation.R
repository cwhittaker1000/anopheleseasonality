...
if (prior == "informative") {
  ...
  fit <- readRDS("Outputs/Logistic_Regression_Output/Uninformative_Prior/Full_Subset_STAN_Output.rds")
}
...
for (i in 1:num_raster_cells) {
  ...
  environmental_raster_values <- t(as.matrix(raster_value_storage[i, ], nrow = 1)) # has to be put here to coerce to row vector
...
saveRDS(temporal_cluster_probabilities, "Outputs/Logistic_Regression_Output/Informative_Prior/Full_Subset_Predictive_Outputs.rds")
temporal_cluster_probabilities <- readRDS("Outputs/Logistic_Regression_Output/Informative_Prior/Full_Subset_Predictive_Outputs.rds")
...
# 8.5 x 7.28
pdf(file = "Figures/Figure_6/Figure_6_Full_Subset_Seasonal_Profile_Predictive_Maps.pdf", height = 7.28, width = 8.5)
